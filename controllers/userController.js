...
module.exports = {
  ...
  getUserPanel: (req, res, next) => {
    ...
    db.query(
      "select * from datPaciente where idDatP =?",
      idDatP,
      (err, rows, fields) => {
        if (err) throw err;
        db.end();
  ...
  calcula: (req, res, next) => {
    ...
    db.beginTransaction((err) => {
      ...
      db.query(sql, idUsuario, (error, results, fields) => {
        ...
        db.query(
          "update datPaciente set imc=" +
            mysql.escape(_imc) +
            ", tmb=" +
            mysql.escape(_tmb) +
            " where datPaciente.idDatP =" +
            mysql.escape(idDatP) +
            ";",
  ...
  postSignInNutri: (req, res, next) => {
    ...
    req.flash(
      "infoN",
      "Nutriologo registrado correctamente, ya puede iniciar sesion"
    );
    return res.redirect("/Login");
  },
  ...
  getNutriCitas: (req, res, next) => {
    console.log(req.user);
    var sql =
      "select paciente.idPaciente, usuario.nombre, usuario.apellidoP, usuario.apellidoM, usuario.email from paciente inner join usuario on paciente.idUsuario = usuario.idUsuario;";
